...
public class CustomerEfRepository(ApplicationDbContext dbContext)
{
    ...
    {
        var item = await _dbContext.Customers.FirstOrDefaultAsync(e => e.Id == model.Id);
        if (item is null)
            return false;
    ...
    {
        var item = await _dbContext.Customers.FirstOrDefaultAsync(e => e.Id == id, cancellationToken);
        if (item is null)
            return false;
